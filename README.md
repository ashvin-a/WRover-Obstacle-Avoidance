...
Make sure to replace "" with whichever version of ros2 is in use (ex: humble) Install DepthAI library using the command:
...
to run via pipline run the following:

echo 'SUBSYSTEM=="usb", ATTRS{idVendor}=="03e7", MODE="0666"' | sudo tee /etc/udev/rules.d/80-movidius.rules
sudo udevadm control --reload-rules && sudo udevadm trigger

and then u must install depthai:
to insall the dependecnies:
